...
**Single-label latency is bound by the slower of the two reads.** The OCR pass and the vision-model call run concurrently, so a check costs the slower one, not the sum. The result page prints the measured time for every check; deployed single-label checks measured 3.4-4.6 seconds, inside the 5-second target, which degrades when a concurrent batch saturates the host CPU — stated because a compliance agent checking one label mid-batch will see it.
