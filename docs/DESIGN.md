...
- Under 5 seconds per label — a stated kill criterion, since an earlier tool's 30–40s meant reviewers abandoned it.
- Handle a batch.
...
- **Net contents verification, and class/type comparison.** Both fields are extracted — the reader quotes them, and they are available at the extractor port — but no rule consumes either for a verdict. A net-contents check is a standards-of-fill question that needs the authorized-container data in 27 CFR part 5 subpart E, and no verification rule for it shipped, so the field is read and carried, not checked or displayed. Class/type routes which rules apply to a label; it is never compared against the filed class/type. Stated so an extracted field is not mistaken for a checked one.
...
The extractor is a `Protocol` with adapters: the hosted vision model, a fixture replay for deterministic tests, and room for a local or in-tenant endpoint. This is the *one* abstraction a stated requirement justifies — a restricted network blocked an earlier tool's cloud ML endpoint (ADR-0004).
