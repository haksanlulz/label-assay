...
- **COLA system integration** — out of scope per the brief.
...
- **One port.** The extractor is swappable (hosted vision model / local OCR / in-tenant endpoint / fixture replay) because a restricted network blocked an earlier tool's cloud endpoint. Other seams are not ports — no stated second implementation, no abstraction.
- **Three-state verdict.** A label that cannot be read routes to a human, never a silent fail.

## Trade-offs and limitations

(Written up at build completion, per the brief's request to document trade-offs.)
